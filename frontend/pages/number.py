...
if "user_id" not in st.session_state or "number_game_state" not in st.session_state:
...
game_state = st.session_state["number_game_state"]
...
if not st.session_state["guess_history"]:
    ...
    if st.button("Start Guessing", key="start_guessing", use_container_width=True):
        api_url = "http://localhost:8000/api/v1/number/select"  # Update as needed
        payload = {
            "user_id": st.session_state["user_id"],
            "user_will": "NUMBER_SELECTED"  # Or whatever your backend expects
        }
        try:
...
for idx, guess in enumerate(st.session_state["guess_history"]):
    ...
    if guess["user_answer"] is None:
        ...
        if user_choice in ["yes", "no"]:
            ...
            st.session_state["guess_history"][idx]["user_answer"] = user_choice
            # Call the API with the answer
            api_url = "http://localhost:8000/api/v1/number/guess"  # Update as needed
            payload = {
                "user_id": st.session_state["user_id"],
